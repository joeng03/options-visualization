...
const cumulativeNormalDistribution = (x) => {
...
const calculateOptionPrice = (type, S, K, T, r, sigma) => {
  ...
  const N_d1 = cumulativeNormalDistribution(d1Value);
  const N_d2 = cumulativeNormalDistribution(d2Value);
  const N_neg_d1 = cumulativeNormalDistribution(-d1Value);
  const N_neg_d2 = cumulativeNormalDistribution(-d2Value);
...
const calculateGreeks = (type, S, K, T, r, sigma) => {
  if (T <= 0) {
    // Handle expiration
    const isInTheMoney =
...
self.onmessage = function (e) {
  ...
  if (task === "calculateGreeks") {
    ...
    self.postMessage({ task: "dataResult", data });
    ...
    self.postMessage({ task: "3dDataResult", data });
    ...
    self.postMessage({ task: "portfolioResult", results });
  }
};
  `;
...
//   r: number,
//   sigma: number
// ): number => d1(S, K, T, r, sigma) - sigma * Math.sqrt(T);
...
// WebAssembly module initialization (assuming a basic WASM export structure)
interface WasmExports {
  ...
  // Add other exports as needed based on your .wasm file
}

const initWasm = async (): Promise<WasmExports> => {
  try {
    const response = await fetch("/options-calc.wasm");
...
const OptionGreeksVisualization: React.FC = () => {
  ...
  // Worker management with queue
  const workerRef = useRef<Worker | null>(null);
  const workerReadyRef = useRef<boolean>(false);
  const messageQueueRef = useRef<WorkerMessage[]>([]);

  // Function to send message or queue it
  const sendWorkerMessage = (message: WorkerMessage) => {
    ...
    console.log(
      `Worker operation "${message.task}" took ${
        endTime - startTime
      } milliseconds`
    );
  };

  // Flush queued messages when worker is ready
  const flushQueue = () => {
    if (workerReadyRef.current && workerRef.current) {
  ...
  useEffect(() => {
    ...
    const calculationWorker = setupWorker();
    workerRef.current = calculationWorker;

    // Set up worker listener and mark as ready
    calculationWorker.onmessage = (e: MessageEvent<WorkerResponse>) => {
      const startTime = performance.now();

      setIsCalculating(false);
      const { task, data, result, results } = e.data;
      ...
      const endTime = performance.now();
      console.log(
        `Worker operation "${task}" took ${
          endTime - startTime
        } milliseconds in onmessage`
      );
    };

    workerReadyRef.current = true;
    console.log("Worker is ready, flushing queue...");
    flushQueue(); // Flush any queued messages once ready
  ...
  // Generate chart data (worker for 3D and portfolio, main thread for 2D)
  useEffect(() => {
  ...
  // In your useEffect where you handle the dataResult task
  useEffect(() => {
  ...
  const calculationMethod = wasmModule ? "WebAssembly" : "JavaScript";
      ...
      <div className="w-full">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-sm bg-blue-100 px-2 py-1 rounded">
            Using: {calculationMethod} {isCalculating && "· Calculating..."}
          </div>
        </div>
        <div className="mb-4">
        ...
        <div className="mt-4 flex justify-end space-x-2">
          ...
          <button
            className="px-3 py-1 bg-gray-600 text-black rounded border border-transparent hover:border-black"
            onClick={async () => {
              const chartElement = document.querySelector(".h-64.w-full");
              if (chartElement) {
                ...
                alert("No chart available to capture.");
              }
            }}
          >
            Take Screenshot
